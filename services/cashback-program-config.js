...
export function deriveProgramMaxCashback(program={}){
  ...
  return conditions.reduce((total,condition)=>{
    const unlimited=condition?.maxCashbackUnlimited===true||condition?.maxType==="UNLIMITED",noCashback=condition?.maxType==="NO_CASHBACK";
    return total+(unlimited||noCashback?0:Math.max(0,Number(condition?.max)||0));
  },0);
}
...
function conditionMarkup(item,index,mode,helpers){
  ...
  const condition=item.condition,noCashback=condition.maxType==="NO_CASHBACK",unlimited=!noCashback&&(condition.maxCashbackUnlimited===true||condition.maxType==="UNLIMITED");
  ...
  return `<article class="cashback-program-condition" data-condition-card ${ref}>
    ...
    <div class="cashback-condition-fields">
      ...
      <label class="field"><span>Giới hạn hoàn</span><select data-condition-max-type><option value="UNLIMITED" ${unlimited?"selected":""}>Không giới hạn</option><option value="LIMITED" ${!unlimited&&!noCashback?"selected":""}>Có giới hạn</option><option value="NO_CASHBACK" ${noCashback?"selected":""}>Không hoàn</option></select></label>
      <label class="field"><span>Max hoàn</span>${moneyInputMarkup({attribute:"data-condition-max",value:unlimited||noCashback?"":formatMoney(condition.max),escape,disabled:unlimited||noCashback})}</label>
...
export function renderCashbackProgramEditor(model={},helpers={}){
  ...
  return `<section class="cashback-program-workflow">${selectors}
    ...
    </div></div><div class="cashback-total-spend-panel"><h3>YÊU CẦU DOANH SỐ</h3><div class="cashback-total-spend-control"><label class="check-field"><input type="checkbox" data-card-total-enabled aria-label="Bật yêu cầu doanh số" ${requiresTotal?"checked":""}></label>${moneyInputMarkup({attribute:"data-card-total-min",value:formatMoney(requirement.amount),escape,disabled:!requiresTotal,ariaLabel:"Tiền yêu cầu tổng doanh số",placeholder:"Tiền yêu cầu tổng doanh số"})}</div></div></section>
    ${packageSection}
