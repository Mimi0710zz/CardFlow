...
import {evaluateCashbackPrograms} from './cashback-evaluation.js?v=20260926-cashback-priority-v1';
...
export function summarizeTrackingCashback(rows=[]){
  ...
  return [...unique.values()].reduce((result,row)=>{if(row.noCashback||row.priorityStatus==='LOCKED_BY_PRIORITY')return result;const amount=Number(row.maxCashback)||0;result.total+=amount;if(row.received)result.received+=amount;return result;},{received:0,total:0});
}
...
export function buildTrackingMatrix(state,{year,month,referenceDate,today=new Date()}={}){
  ...
  const normalPrograms=programs.filter(program=>!isMbPlatinumCard(program.cardId)),normalEvaluations=new Map(evaluateCashbackPrograms(normalPrograms,state.transactions,state.cards,{mccCategories:state.mccCategories||[],referenceDate:referenceDate||getCashbackReferenceDate(year,month),cardCashbackConfigs:state.cashbackCardConfigs||[]}).map(result=>[result.group.id,result]));
  ...
  const rows=programs.map(program=>{
    ...
    const locked=evaluation.priorityStatus==='LOCKED_BY_PRIORITY',completed=evaluation.overallSatisfied&&!locked,status=locked?'LOCKED_BY_PRIORITY':(completed?'COMPLETED':(evaluation.totalSpend>0||eligibleSpend>0?'IN_PROGRESS':'AVAILABLE')),deadline=trackingDeadline(evaluation.period,today,completed),note=String(program.note||'');
    ...
    return {cardId:card.id,bank,card,cardTemplate:card.network||'',program,programId:program.id,programName:program.name,periodType,periodKey,eligibleSpend,totalSpend:evaluation.totalSpend,deadline,maxCashback,cashbackCycle:periodType==='STATEMENT'?'statement':'monthly',received:!locked&&receipt?.received===true,receivedDate:locked?'':(receipt?.receivedDate||''),receiptEnabled:!locked&&!noCashback,noCashback,note,priorityStatus:evaluation.priorityStatus||'ACTIVE',competitionWinnerId:evaluation.competitionWinnerId||'',metric};
  });
