...
assert.match(app,/function closeCashbackMccOnOutsideClick\(event\)/);
assert.match(app,/document\.addEventListener\("click",closeCashbackMccOnOutsideClick\)/);
assert.match(app,/\.cashback-mcc-select\.open/);
assert.match(app,/restoreCashbackProgramSnapshot/);
assert.match(app,/data-card-total-min[^\n]*disabled/);
assert.match(app,/data-condition-rate[^\n]*blur/);
assert.match(app,/recalculateCashbackProgramMax/);
assert.match(app,/function wireCashbackProgramInputUx\(root\)/);
assert.match(app,/event=>event\.currentTarget\.select\?\.\(\)/);
assert.match(app,/\.cashback-money-input input/);
assert.match(app,/formatMoneyInput\(input\.value,\{allowEmpty:true\}\)/);
...
assert.match(css,/\.cashback-total-spend-control\{[^}]*display:inline-grid[^}]*grid-template-columns:18px minmax\(220px,280px\)[^}]*column-gap:10px[^}]*justify-self:start[^}]*width:max-content/);
assert.match(css,/\.cashback-total-spend-control \.check-field\{[^}]*width:18px[^}]*min-width:18px/);
assert.match(css,/\.cashback-total-spend-control \.cashback-money-input\{[^}]*width:280px[^}]*max-width:100%/);
