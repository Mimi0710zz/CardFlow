...
assert.match(simpleHtml,/data-card-total-min[^>]*disabled/);
...
const totalSpendHtml=renderCashbackProgramEditor(buildCashbackProgramEditorModel({
  cards,
  programs:[simple],
  cardCashbackConfigs:[{cardId:"CARD-A",calculationMode:"independent",totalSpendRequirement:{enabled:true,amount:5000000}}],
  selection:{cardId:"CARD-A",programId:"SIMPLE"}
}),helpers);
assert.match(totalSpendHtml,/data-card-total-enabled[^>]*checked/);
assert.match(totalSpendHtml,/data-card-total-min[^>]*value="5000000"/);
assert.doesNotMatch(totalSpendHtml,/data-card-total-min[^>]*disabled/);
