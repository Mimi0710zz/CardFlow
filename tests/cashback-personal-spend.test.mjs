...
const transactions=[
  ...
  {id:"BACK",cardId:"TECH Every",date:"2026-09-03",status:TRANSACTION_STATUS.HOST_BACK,host:"HOST",amount:3000000,mcc:"5411",mccCategoryId:"MCC-5411",channel:"Online",backAmount:2900000},
  {id:"PERSONAL",cardId:"TECH Every",date:"2026-09-04",status:TRANSACTION_STATUS.PERSONAL_USE,host:"",amount:5000000,mcc:"5411",mccCategoryId:"MCC-5411",channel:"Online",backAmount:0}
];
...
const cell=trackingRow.cells[0];
...
assert.deepEqual(trackingRow.cells.map(item=>item.total),[11000000,11000000]);
...
const wrongMccCell=buildTrackingMatrix({...baseState,transactions:[...transactions,wrongMcc]},{year:2026,month:9,referenceDate:"2026-09-14"}).rows[0].cells[0];
...
const bugCell=buildTrackingMatrix({...baseState,transactions:[...transactions,bugLazada]},{year:2026,month:9,referenceDate:"2026-09-14"}).rows[0].cells[0];
...
assert.equal(hostMetrics.hostBack,2900000);
