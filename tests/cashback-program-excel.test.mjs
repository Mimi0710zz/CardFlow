...
const programs=[
  {id:"SIMPLE",cardId:"CARD",name:"Simple Program",year:2026,month:9,conditionMode:"supporting",totalSpendMinimum:5000000,maxCashbackPerPeriod:300000,conditionCombination:"AND",conditions:[condition("S1","First"),condition("S2","Second")]},
...
assert.equal(rows.filter(row=>row["Program ID"]==="SIMPLE").every(row=>row["Condition Mode"]==="supporting"),true);
assert.equal(rows.find(row=>row["Program ID"]==="PACKAGED")["Condition Mode"],"supporting");
...
assert.equal(importedSimple.conditionMode,"supporting");
assert.equal(importedPackaged.conditionMode,"supporting");
