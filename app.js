...
function cashbackConditionRow(condition,index,program){
  ...
  return `<div class="cashback-condition-row" data-condition-row data-condition-id="${esc(normalized.id)}">
    ...
    <div class="cashback-condition-cell" data-label="Chi nhóm tối thiểu"><div class="money-input"><input data-condition-min inputmode="numeric" value="${esc(minSpend)}"><span>đ</span></div><input data-condition-note value="${esc(normalized.note||"")}" placeholder="Ghi chú điều kiện"></div>
...
async function openCashbackProgramForm(title,existing={}){
  ...
  body.innerHTML=`<h3 class="cashback-section-title">THÔNG TIN CHUNG</h3><div class="cashback-program-top"><div class="field"><label>Tên chương trình</label><input name="name" required value="${esc(existing.name||"")}"></div><div class="field"><label>Card ID</label><select name="cardId">${selectOptions(state.cards,card=>card.id).map(option=>`<option value="${esc(option.value)}" ${option.value===(existing.cardId||state.cards[0]?.id)?"selected":""}>${esc(option.label)}</option>`).join("")}</select></div></div>
    ...
    <div data-legacy-editor class="${packaged?"hidden":""}"><section class="cashback-conditions"><div class="cashback-condition-header"><span>Tên điều kiện</span><span>MCC</span><span>Hình thức giao dịch</span><span>Tỷ lệ hoàn (%)</span><span>Hoàn tối đa</span><span>Chi nhóm tối thiểu / Ghi chú</span></div><div data-condition-list>${initialConditions.map((condition,index)=>cashbackConditionRow(condition,index,existing)).join("")}</div></section><button type="button" class="secondary-btn cashback-add-condition" data-add-condition>+ Thêm điều kiện</button><div class="cashback-total-row"><label class="field">Điều kiện kết hợp<select data-group-combination><option value="AND" ${normalizeCombineOperator(existing.conditionCombination)==="AND"?"selected":""}>AND</option><option value="OR" ${normalizeCombineOperator(existing.conditionCombination)==="OR"?"selected":""}>OR</option></select></label><div class="field"><label>Ghi chú chung</label><input data-group-note value="${esc(existing.note||"")}"></div></div></div>
  ...
  const recalculate=row=>{const unlimited=row.querySelector("[data-condition-max-type]").value==="UNLIMITED",maxInput=row.querySelector("[data-condition-max]"),minInput=row.querySelector("[data-condition-min]");maxInput.disabled=unlimited;maxInput.placeholder=unlimited?"Không áp dụng":"0";if(unlimited)maxInput.value="";else maxInput.value=formatMoneyInput(maxInput.value,{allowEmpty:true});minInput.value=formatMoneyInput(minInput.value,{allowEmpty:true});};
...
function cashbackGroupEditor(group={},packageIndex=0,groupIndex=0){
  ...
  return `<section class="cashback-package-group" data-package-group data-group-id="${esc(group.id||`GROUP-${packageIndex+1}-${groupIndex+1}`)}"><div class="cashback-package-head"><input data-group-name value="${esc(group.name||`Nhóm ${groupIndex+1}`)}" aria-label="Tên nhóm"><button type="button" class="icon-btn" data-delete-group title="Xóa nhóm">${icon("trash")}</button></div><div class="cashback-conditions"><div class="cashback-condition-header"><span>Tên điều kiện</span><span>MCC</span><span>Hình thức giao dịch</span><span>Tỷ lệ hoàn (%)</span><span>Hoàn tối đa</span><span>Chi nhóm tối thiểu / Ghi chú</span></div><div data-condition-list>${conditions.map((condition,index)=>cashbackConditionRow(condition,index,group)).join("")}</div></div><button type="button" class="secondary-btn cashback-add-condition" data-add-condition>+ Thêm điều kiện</button><div class="cashback-total-row"><label class="field">Chi tổng nhóm<input data-group-total inputmode="numeric" value="${esc(formatMoneyInput(group.totalSpendMinimum,{allowEmpty:true}))}"></label><label class="field">Kết hợp<select data-group-combination><option ${normalizeCombineOperator(group.conditionCombination)==="AND"?"selected":""}>AND</option><option ${normalizeCombineOperator(group.conditionCombination)==="OR"?"selected":""}>OR</option></select></label><label class="field">Ghi chú<input data-group-note value="${esc(group.note||"")}"></label></div></section>`;
}
