...
import { mountTrackingMatrix } from "./services/tracking-matrix-ui.js?v=20260926-cashback-priority-v1";
import { normalizeTrackingCashbackReceipts, upsertTrackingCashbackReceipt } from "./services/tracking-cashback-receipts.js";
import { evaluateCashbackPrograms } from "./services/cashback-evaluation.js?v=20260926-cashback-priority-v1";
...
function programMetrics(){
  return evaluateCashbackPrograms(programs(),state.transactions,state.cards,{mccCategories:state.mccCategories,referenceDate:cashbackReferenceDate(),cardCashbackConfigs:state.cashbackCardConfigs}).map(result=>{
    ...
    return {...result.group,id:result.group.id,conditions,combineOperator:result.conditionCombination||"OR",
      eligible:sum(conditions,item=>item.eligibleSpend),total:result.totalSpend,rawCashback:result.totalCashback,
      countedCashback:result.totalCashback,displayCashback:result.totalCashback,
      eligibleTarget:sum(conditions,item=>item.eligibleTarget)||null,totalTarget:result.totalSpendMinimum,
      remainEligible:remainValues.length?Math.min(...remainValues):null,
      remainTotal:result.totalSpendMinimum==null?null:Math.max(0,result.totalSpendMinimum-result.totalSpend),
      progress:result.progress,combinationSatisfied:result.overallSatisfied,evaluation:result};
  });
}
...
function responsiveAccordionTitle(entity,id,row){
  ...
  if(entity==="programs"){
    const program=state.cashbackProgramGroups.find(item=>item.id===id);
...
async function openForm(title, fields, initial = {}, onRender = null){
  ...
  body.innerHTML = fields.map(f => {
    ...
    const inputAttrs = `name="${esc(f.name)}" type="${esc(inputType)}" value="${esc(inputValue)}" ${f.kind==="money"?'inputmode="numeric" autocomplete="off"':""} ${f.placeholder?`placeholder="${esc(f.placeholder)}"`:""} ${f.min!=null?`min="${esc(f.min)}"`:""} ${f.max!=null?`max="${esc(f.max)}"`:""} ${f.step?`step="${esc(f.step)}"`:""} ${f.required?"required":""} ${f.readonly?"readonly":""} ${disabledAttr}`;
...
function cardFields(card={}, mode="add"){
  ...
  return [
    ...
    {name:"paymentTermDays", label:"Số ngày thanh toán", value:normalizePaymentTermDays(card.paymentTermDays) ?? "", type:"number", min:1, step:1, placeholder:"Nhập số ngày", required:true},
...
function validateCard(values, existingId=""){
  ...
  if(cardType === "credit" && paymentTermDays == null) return {error:"Số ngày thanh toán phải là số nguyên dương."};
...
function normalizeProgramValues(values, existing={}){
  ...
  const id=existing.id || uniqueCashbackProgramId(baseId,state.cashbackProgramGroups);
...
function syncCashbackCombineOperator(program){
  ...
  state.cashbackProgramGroups.forEach(item=>{
...
function cashbackConditionRow(condition,index,program){
  ...
  const minSpend=formatMoneyInput(normalized.eligibleSpendMinimum??normalized.eligibleTarget,{allowEmpty:true});
  return `<div class="cashback-condition-row" data-condition-row data-condition-id="${esc(normalized.id)}">
    <div class="cashback-condition-index" data-label="Điều kiện"><input data-condition-name value="${esc(normalized.name||`Điều kiện ${index+1}`)}" aria-label="Tên điều kiện"></div>
    ...
    <div class="cashback-condition-cell cashback-condition-min-auto" data-label="Chi để đạt Max CB"><div class="money-input"><input data-condition-min inputmode="numeric" value="${esc(minSpend)}" placeholder="Không áp dụng" readonly><span>đ</span></div></div>
    ...
    <div class="cashback-condition-note-row"><label>Ghi chú điều kiện</label><input data-condition-note value="${esc(normalized.note||"")}" placeholder="Nhập ghi chú điều kiện (nếu có)"></div>
  </div>`;
}
function recalculateCashbackConditionRow(row){
  const unlimited=row.querySelector("[data-condition-max-type]").value==="UNLIMITED";
  const rate=parseCashbackRateInput(row.querySelector("[data-condition-rate]").value);
  const maxInput=row.querySelector("[data-condition-max]");
  const minInput=row.querySelector("[data-condition-min]");
  maxInput.disabled=unlimited;
  maxInput.placeholder=unlimited?"Không áp dụng":"0";
  if(unlimited){
    maxInput.value="";
    minInput.value="";
    minInput.placeholder="Không áp dụng";
    return;
  }
  maxInput.value=formatMoneyInput(maxInput.value,{allowEmpty:true});
  const max=parseMoney(maxInput.value,{emptyValue:0});
  const spend=calculateSpendToMax(rate,max);
  minInput.value=spend==null?"":formatMoneyInput(spend,{allowEmpty:true});
  minInput.placeholder=spend==null?"Không áp dụng":"0";
}
function collectCashbackCondition(row){
  ...
  const eligibleSpendMinimum=parseMoney(row.querySelector("[data-condition-min]").value,{emptyValue:null});
  return {id:row.dataset.conditionId,name:String(row.querySelector("[data-condition-name]").value||""),allMcc,mccCategoryIds,categories:mccCategoryIds.map(id=>state.mccCategories.find(item=>item.id===id)?.name).filter(Boolean),channel:normalizeTransactionMethod(row.querySelector("[data-condition-channel]").value),rate,max,maxCashbackUnlimited,maxType:maxCashbackUnlimited?"UNLIMITED":"LIMITED",eligibleSpendMinimum,eligibleTarget:eligibleSpendMinimum,note:String(row.querySelector("[data-condition-note]").value||"")};
}
...
async function openCashbackProgramForm(title,existing={}){
  ...
  const totalCondition={enabled:existing.totalSpendMinimum!=null,amount:existing.totalSpendMinimum};
  modal.querySelector("h2").textContent=title;
  ...
  const packaged=hasCashbackPackages(existing);
  body.innerHTML=`<h3 class="cashback-section-title">THÔNG TIN CHUNG</h3><div class="cashback-program-top"><div class="field"><label>Tên chương trình</label><input name="name" required value="${esc(existing.name||"")}"></div><div class="field"><label>Card ID</label><select name="cardId">${selectOptions(state.cards,card=>card.id).map(option=>`<option value="${esc(option.value)}" ${option.value===(existing.cardId||state.cards[0]?.id)?"selected":""}>${esc(option.label)}</option>`).join("")}</select></div></div>
    <h3 class="cashback-section-title">ĐIỀU KIỆN TOÀN CHƯƠNG TRÌNH</h3><div class="cashback-program-rules"><label class="field">Tổng chi tối thiểu<input data-program-min inputmode="numeric" value="${esc(formatMoneyInput(existing.totalSpendMinimum,{allowEmpty:true}))}"></label><label class="field">Max cashback toàn kỳ<input data-program-max inputmode="numeric" value="${esc(formatMoneyInput(existing.maxCashbackPerPeriod,{allowEmpty:true}))}"></label><label class="check-field"><input type="checkbox" data-package-mode ${packaged?"checked":""}><span>Có sử dụng Gói hoàn tiền</span></label><label class="field">Số lần đổi gói tối đa<input type="number" min="0" data-switch-limit value="${Number(existing.packageSwitchLimit)||0}" ${packaged?"":"disabled"}></label></div>
    <div data-legacy-editor class="${packaged?"hidden":""}"><section class="cashback-conditions"><div class="cashback-condition-header"><span>Tên điều kiện</span><span>MCC</span><span>Hình thức giao dịch</span><span>Tỷ lệ hoàn (%)</span><span>Hoàn tối đa</span><span>Chi để đạt Max CB</span></div><div data-condition-list>${initialConditions.map((condition,index)=>cashbackConditionRow(condition,index,existing)).join("")}</div></section><button type="button" class="secondary-btn cashback-add-condition" data-add-condition>+ Thêm điều kiện</button><div class="cashback-total-row"><label class="field">Điều kiện kết hợp<select data-group-combination><option value="AND" ${normalizeCombineOperator(existing.conditionCombination)==="AND"?"selected":""}>AND</option><option value="OR" ${normalizeCombineOperator(existing.conditionCombination)==="OR"?"selected":""}>OR</option></select></label><div class="field"><label>Ghi chú chung</label><input data-group-note value="${esc(existing.note||"")}"></div></div></div>
    <div data-package-editor class="${packaged?"":"hidden"}"><h3 class="cashback-section-title">GÓI HOÀN TIỀN</h3><div data-package-list>${(existing.packages?.length?existing.packages:[{}]).map(cashbackPackageEditor).join("")}</div><button type="button" class="secondary-btn" data-add-package>+ Thêm gói</button></div>`;
  let operator=normalizeCombineOperator(existing.conditionCombination),openMcc=null;
  const list=body.querySelector("[data-legacy-editor] [data-condition-list]");
  ...
  const refreshRows=()=>{const rows=[...list.querySelectorAll("[data-condition-row]")];rows.forEach((row,index)=>{row.querySelector(".cashback-condition-action").innerHTML=index?`<button type="button" class="icon-btn cashback-delete-condition" title="Xóa điều kiện" aria-label="Xóa điều kiện">${icon("trash")}</button>`:"";});};
  const wireRow=row=>{const mcc=row.querySelector(".cashback-mcc-select"),toggle=mcc.querySelector("[data-cashback-mcc-toggle]"),boxes=[...mcc.querySelectorAll('input[type="checkbox"]')];toggle.onclick=()=>toggleMcc(mcc);boxes.forEach(box=>box.onchange=()=>{if(box.value===ALL_MCC_VALUE&&box.checked)boxes.forEach(other=>{other.checked=other===box;});else if(box.checked)boxes.find(other=>other.value===ALL_MCC_VALUE).checked=false;toggle.textContent=cashbackMccSummary(row);});const rate=row.querySelector("[data-condition-rate]"),max=row.querySelector("[data-condition-max]");[...row.querySelectorAll("[data-condition-rate],[data-condition-max],[data-condition-max-type]")].forEach(input=>input.addEventListener("input",()=>recalculateCashbackConditionRow(row)));rate.onblur=()=>{rate.value=cashbackRateInput(parseCashbackRateInput(rate.value));recalculateCashbackConditionRow(row);};max.onblur=()=>recalculateCashbackConditionRow(row);row.querySelector("[data-condition-max-type]").onchange=()=>recalculateCashbackConditionRow(row);recalculateCashbackConditionRow(row);};
  body.querySelectorAll("[data-condition-row]").forEach(wireRow);
  body.onclick=event=>{const operatorButton=event.target.closest("[data-operator]");if(operatorButton){operator=operatorButton.dataset.operator;body.querySelectorAll("[data-operator]").forEach(button=>button.classList.toggle("active",button.dataset.operator===operator));return;}const addCondition=event.target.closest("[data-add-condition]");if(addCondition){const target=addCondition.closest("[data-package-group]")?.querySelector("[data-condition-list]")||list,index=target.querySelectorAll("[data-condition-row]").length;target.insertAdjacentHTML("beforeend",cashbackConditionRow({},index,{id:existing.id||"PROGRAM"}));wireRow(target.lastElementChild);refreshRows();return;}const remove=event.target.closest(".cashback-delete-condition");if(remove){const target=remove.closest("[data-condition-list]");if(target.querySelectorAll("[data-condition-row]").length>1)remove.closest("[data-condition-row]").remove();return;}if(event.target.closest("[data-add-package]")){const target=body.querySelector("[data-package-list]"),index=target.querySelectorAll(":scope > [data-package]").length;target.insertAdjacentHTML("beforeend",cashbackPackageEditor({id:prefixedUuid("CBP")},index));target.lastElementChild.querySelectorAll("[data-condition-row]").forEach(wireRow);return;}if(event.target.closest("[data-add-group]")){const pkg=event.target.closest("[data-package]"),target=pkg.querySelector("[data-group-list]"),index=target.querySelectorAll(":scope > [data-package-group]").length;target.insertAdjacentHTML("beforeend",cashbackGroupEditor({id:prefixedUuid("CBG")},0,index));target.lastElementChild.querySelectorAll("[data-condition-row]").forEach(wireRow);return;}const deleteGroup=event.target.closest("[data-delete-group]");if(deleteGroup&&deleteGroup.closest("[data-group-list]").querySelectorAll(":scope > [data-package-group]").length>1){deleteGroup.closest("[data-package-group]").remove();return;}const deletePackage=event.target.closest("[data-delete-package]");if(deletePackage&&body.querySelectorAll("[data-package]").length>1)deletePackage.closest("[data-package]").remove();};
  const packageMode=body.querySelector("[data-package-mode]");packageMode.onchange=()=>{body.querySelector("[data-legacy-editor]").classList.toggle("hidden",packageMode.checked);body.querySelector("[data-package-editor]").classList.toggle("hidden",!packageMode.checked);body.querySelector("[data-switch-limit]").disabled=!packageMode.checked;};body.querySelectorAll("[data-program-min],[data-program-max],[data-group-total]").forEach(input=>input.oninput=()=>input.value=formatMoneyInput(input.value,{allowEmpty:true}));
  ...
  modal.classList.add("show");
  return new Promise(resolve=>{const close=result=>{modal.classList.remove("show");form.onsubmit=null;body.onclick=null;form.classList.remove("cashback-program-modal");resolve(result);};modal.querySelector("[data-cancel-modal]").onclick=()=>close(null);form.onsubmit=event=>{event.preventDefault();const name=body.querySelector('[name="name"]').value.trim(),cardId=body.querySelector('[name="cardId"]').value;if(!name)return toast("Vui lòng nhập tên chương trình.");if(!cardId)return toast("Vui lòng chọn thẻ.");const validate=conditions=>{if(conditions.some(item=>!item.name.trim()||(!item.allMcc&&!item.mccCategoryIds.length)||item.rate<=0||(!item.maxCashbackUnlimited&&item.max<=0)))return false;return true;};const common={name,cardId,totalSpendMinimum:parseMoney(body.querySelector("[data-program-min]").value,{emptyValue:null}),maxCashbackPerPeriod:parseMoney(body.querySelector("[data-program-max]").value,{emptyValue:null}),packageSwitchLimit:Number(body.querySelector("[data-switch-limit]").value)||0};if(packageMode.checked){const packages=[...body.querySelectorAll("[data-package]")].map((section,packageIndex)=>({id:section.dataset.packageId.startsWith("PACKAGE-")?`${existing.id||"PROGRAM"}-PACKAGE-${packageIndex+1}`:section.dataset.packageId,name:section.querySelector("[data-package-name]").value.trim(),groups:[...section.querySelectorAll(":scope > [data-group-list] > [data-package-group]")].map(collectPackageGroup)}));if(packages.some(pkg=>!pkg.name||pkg.groups.some(group=>!group.name||!validate(group.conditions))))return toast("Vui lòng nhập đủ tên gói, nhóm và điều kiện hợp lệ.");return close({...common,packages,packageHistory:existing.packageHistory||[]});}const conditions=[...list.querySelectorAll("[data-condition-row]")].map(collectCashbackCondition);if(!validate(conditions))return toast("Vui lòng nhập đầy đủ điều kiện cashback hợp lệ.");close({...common,conditionCombination:body.querySelector("[data-group-combination]").value,note:body.querySelector("[data-group-note]").value,conditions,totalSpendCondition:{enabled:common.totalSpendMinimum!=null,amount:common.totalSpendMinimum}});};});
}
function normalizeCashbackProgramFormValues(values,existing={}){
  const name=String(values.name||"");
  const id=existing.id||uniqueCashbackProgramId(buildCashbackProgramId(values.cardId,name),state.cashbackProgramGroups);
  if(values.packages){
    return {...existing,id,year:selectedYear,month:selectedMonth,name,cardId:values.cardId,totalSpendMinimum:values.totalSpendMinimum,maxCashbackPerPeriod:values.maxCashbackPerPeriod,packageSwitchLimit:values.packageSwitchLimit,packages:values.packages,packageHistory:values.packageHistory||[]};
  }
  const totalTarget=values.totalSpendCondition.enabled?values.totalSpendCondition.amount:null;
  const used=new Set();
  const conditions=values.conditions.map((condition,index)=>{let conditionId=String(condition.id||"");if(!existing.id||conditionId.startsWith("PROGRAM-COND-"))conditionId=`${id}-COND-${index+1}`;let candidate=conditionId,suffix=2;while(used.has(candidate)){candidate=`${conditionId}-${suffix++}`;}used.add(candidate);return {...condition,id:candidate};});
  const result={...existing,id,year:selectedYear,month:selectedMonth,name,cardId:values.cardId,conditionCombination:normalizeCombineOperator(values.conditionCombination),totalSpendMinimum:totalTarget,maxCashbackPerPeriod:values.maxCashbackPerPeriod,packageSwitchLimit:0,note:String(values.note||""),conditions};
  delete result.packages;delete result.packageHistory;
  return result;
}
function cashbackGroupEditor(group={},packageIndex=0,groupIndex=0){
  const conditions=normalizeCashbackConditions(group,state.mccCategories);
  return `<section class="cashback-package-group" data-package-group data-group-id="${esc(group.id||`GROUP-${packageIndex+1}-${groupIndex+1}`)}"><div class="cashback-package-head"><input data-group-name value="${esc(group.name||`Nhóm ${groupIndex+1}`)}" aria-label="Tên nhóm"><button type="button" class="icon-btn" data-delete-group title="Xóa nhóm">${icon("trash")}</button></div><div class="cashback-conditions"><div class="cashback-condition-header"><span>Tên điều kiện</span><span>MCC</span><span>Hình thức giao dịch</span><span>Tỷ lệ hoàn (%)</span><span>Hoàn tối đa</span><span>Chi để đạt Max CB</span></div><div data-condition-list>${conditions.map((condition,index)=>cashbackConditionRow(condition,index,group)).join("")}</div></div><button type="button" class="secondary-btn cashback-add-condition" data-add-condition>+ Thêm điều kiện</button><div class="cashback-total-row"><label class="field">Chi tổng nhóm<input data-group-total inputmode="numeric" value="${esc(formatMoneyInput(group.totalSpendMinimum,{allowEmpty:true}))}"></label><label class="field">Kết hợp<select data-group-combination><option ${normalizeCombineOperator(group.conditionCombination)==="AND"?"selected":""}>AND</option><option ${normalizeCombineOperator(group.conditionCombination)==="OR"?"selected":""}>OR</option></select></label><label class="field">Ghi chú<input data-group-note value="${esc(group.note||"")}"></label></div></section>`;
}
function cashbackPackageEditor(pkg={},packageIndex=0){
  const groups=pkg.groups?.length?pkg.groups:[{}];
  return `<section class="cashback-package" data-package data-package-id="${esc(pkg.id||`PACKAGE-${packageIndex+1}`)}"><div class="cashback-package-head"><input data-package-name value="${esc(pkg.name||`Gói ${packageIndex+1}`)}" aria-label="Tên gói"><button type="button" class="icon-btn" data-delete-package title="Xóa gói">${icon("trash")}</button></div><div data-group-list>${groups.map((group,index)=>cashbackGroupEditor(group,packageIndex,index)).join("")}</div><button type="button" class="secondary-btn" data-add-group>+ Thêm nhóm</button></section>`;
}
function collectPackageGroup(section){
  const conditions=[...section.querySelectorAll(":scope > .cashback-conditions [data-condition-row]")].map(collectCashbackCondition);
  return {id:section.dataset.groupId,name:section.querySelector("[data-group-name]").value,totalSpendMinimum:parseMoney(section.querySelector("[data-group-total]").value,{emptyValue:null}),conditionCombination:section.querySelector("[data-group-combination]").value,note:section.querySelector("[data-group-note]").value,conditions};
}
function cashbackProgramCard(program){
  return state.cards.find(item=>item.id===program?.cardId);
}
function cashbackProgramBankName(program){
  const card=cashbackProgramCard(program);
  return bankName(card?.bankId,card?.bank||"—");
}
...
async function openCashbackRuntimeForm(title,fields,submitLabel){
  const submit=document.querySelector('#formModal button[type="submit"]');
  const previousLabel=submit?.textContent;
  try{
    return await openForm(title,fields,{},()=>{if(submit)submit.textContent=submitLabel;});
  }finally{
    if(submit)submit.textContent=previousLabel;
  }
}
async function manageCashbackPackage(program){
  if(!hasCashbackPackages(program))return;
  ...
  if(view.action==="initialize"){
    ...
    if(view.switchDisabled)return toast(`Đã sử dụng ${view.switchCount}/${view.switchLimit} lần đổi gói trong kỳ sao kê này.`);
    values=await openCashbackRuntimeForm("ĐỔI GÓI HOÀN TIỀN",[
      {type:"note",label:`Gói hiện tại: ${view.activePackage.name}`},
      {name:"packageId",label:"Đổi sang",type:"select",options:program.packages.filter(pkg=>pkg.id!==view.activePackage.id).map(pkg=>({value:pkg.id,label:pkg.name})),required:true},
      {type:"note",label:`Thời điểm hiệu lực: ${formatCashbackRuntimeTimestamp(timestamp)}`},
      {type:"note",label:`Đã đổi: ${view.switchCount} / ${view.switchLimit} lần`},
      {type:"note",label:"Các giao dịch trước thời điểm đổi gói vẫn được tính theo gói cũ."}
    ],"Xác nhận đổi gói");
    if(!values)return;
    result=switchCashbackPackage(program,values.packageId,timestamp,card);
  }
  if(result.error)return toast(result.error);
  const index=state.cashbackProgramGroups.findIndex(item=>item.id===program.id);state.cashbackProgramGroups[index]=result.program;saveState(successMessage);
}
function cashbackPackageRuntimeMarkup(program){
  if(!hasCashbackPackages(program))return "";
  const view=buildCashbackPackageRuntimeView(program,cashbackProgramCard(program),cashbackRuntimeTimestamp());
...
function renderCashbackRuntimePanel(programId=selectedRows.programs){
  ...
  if(!root)return;
  const program=state.cashbackProgramGroups.find(item=>item.id===programId);
...
function renderProgramsLegacy(){
  ...
  const groupRule=group=>group.totalTarget==null?"Không có":`Tổng chi ≥ ${formatMoneyDisplay(group.totalTarget)} (${group.combineOperator})`;
  const showPackages=groups.some(hasCashbackPackages);
  document.querySelector("#view-programs").innerHTML=`<div class="card"><div class="section-title"><h2>Chương trình cashback</h2><small>Nhóm chương trình và các điều kiện hoàn tiền dùng chung một rule tổng.</small></div>${toolbar("programs")}<div data-cashback-runtime-root></div><div class="table-wrap"><table class="cashback-program-table" data-entity="programs"><thead><tr><th>Ngân hàng</th><th>Thẻ</th><th>Chương trình</th>${showPackages?"<th>Gói hoàn tiền</th><th>Nhóm cashback</th>":""}<th>Điều kiện cashback</th><th>Điều kiện chung</th><th>Tỷ lệ</th><th>Max cashback</th><th>Chi nhóm tối thiểu</th><th>Hình thức</th><th>MCC</th><th>Ghi chú</th><th>Cashback</th></tr></thead><tbody>${rows.map((row,index)=>{const {group,condition}=row,card=cashbackProgramCard(group),color=bankTextColor(card),groupStart=index===0||rows[index-1].group.id!==group.id,bankStart=index===0||cashbackProgramBankName(rows[index-1].group)!==cashbackProgramBankName(group),cardStart=index===0||rows[index-1].group.cardId!==group.cardId;return `<tr data-id="${esc(group.id)}" class="${selectedRows.programs===group.id?"selected":""}">${bankStart?`<td rowspan="${spanFrom(index,(a,b)=>cashbackProgramBankName(a.group)===cashbackProgramBankName(b.group))}" class="cashback-bank-cell" style="color:${esc(color)}">${esc(cashbackProgramBankName(group))}</td>`:""}${cardStart?`<td rowspan="${spanFrom(index,(a,b)=>a.group.cardId===b.group.cardId)}" class="cashback-bank-cell" style="color:${esc(color)}">${esc(group.cardId)}</td>`:""}${groupStart?`<td rowspan="${group.conditions.length}" class="wrap-cell cashback-program-name-cell">${esc(group.name)}</td>`:""}${showPackages?`<td class="wrap-cell">${esc(condition.packageName||"—")}</td><td class="wrap-cell">${esc(condition.groupName||"—")}</td>`:""}<td class="wrap-cell">${esc(condition.name)}</td>${groupStart?`<td rowspan="${group.conditions.length}" class="wrap-cell">${esc(groupRule(group))}</td>`:""}<td>${formatCashbackRate(condition.rate)}</td><td class="num">${isCashbackUnlimited(condition)?"Không giới hạn":formatMoneyDisplay(condition.max)}</td><td class="num">${optionalMoneyDisplay(condition.eligibleTarget)}</td><td>${esc(cashbackTransactionMethodLabel(condition.channel))}</td><td class="wrap-cell">${esc(mccProgramSummary(condition))}</td><td class="wrap-cell">${esc([condition.note,group.note].filter(Boolean).join(" · ")||"—")}</td><td class="num">${formatMoneyDisplay(condition.finalCashback)}</td></tr>`;}).join("")}</tbody></table></div></div>`;
  wireToolbar("programs", {
    add: async()=>{ const values=await openCashbackProgramForm("Thêm nhóm chương trình cashback");if(!values)return;const program=normalizeCashbackProgramFormValues(values);state.cashbackProgramGroups.push(program);selectedRows.programs=program.id;saveState("Đã thêm nhóm chương trình"); },
    edit: async id=>{ const i=state.cashbackProgramGroups.findIndex(x=>x.id===id);const existing=normalizedProgramForDisplay(state.cashbackProgramGroups[i]);const values=await openCashbackProgramForm("Chỉnh sửa nhóm chương trình cashback",existing);if(!values)return;state.cashbackProgramGroups[i]=normalizeCashbackProgramFormValues(values,existing);selectedRows.programs=id;saveState("Đã cập nhật nhóm chương trình"); },
    remove: id=>{ if(!confirm("Xóa toàn bộ nhóm chương trình cashback đã chọn?")) return; state.cashbackProgramGroups=state.cashbackProgramGroups.filter(x=>x.id!==id); clearRowSelection("programs"); saveState("Đã xóa nhóm chương trình"); },
    bulkRemove:ids=>{const selected=new Set(ids);state.cashbackProgramGroups=state.cashbackProgramGroups.filter(program=>!selected.has(program.id));clearRowSelection("programs");saveState(`Đã xóa ${ids.length} nhóm chương trình cashback`);}
  });
  document.querySelectorAll('#view-programs [data-entity="programs"] tr[data-id]').forEach(row=>row.addEventListener("click",()=>renderCashbackRuntimePanel(row.dataset.id)));
  renderCashbackRuntimePanel();
}
...
function cashbackConditionValues(card){
  ...
  const maxType=card.querySelector("[data-condition-max-type]").value,unlimited=maxType==="UNLIMITED",noCashback=maxType==="NO_CASHBACK";
  return {
    ...
    max:unlimited||noCashback?null:parseMoney(card.querySelector("[data-condition-max]").value,{emptyValue:0}),
    maxCashbackUnlimited:unlimited,maxType,
...
function recalculateCashbackProgramCondition(card){
  const maxType=card.querySelector("[data-condition-max-type]").value,unlimited=maxType==="UNLIMITED",noCashback=maxType==="NO_CASHBACK",maxInput=card.querySelector("[data-condition-max]"),target=card.querySelector("[data-condition-spend-to-max]");
  maxInput.disabled=unlimited||noCashback;
  if(unlimited||noCashback){maxInput.value="";target.value="Không áp dụng";return;}
...
function collectCashbackProgramDraft(root,program){
  ...
  const draft={...program,name:String(root.querySelector("[data-program-name]")?.value||program.name||"").trim()};
...
function wireCashbackProgramEditor(model){
  ...
  root.querySelector("[data-delete-program]")?.addEventListener("click",()=>{const program=selectedCashbackProgram();if(!program||!confirm(`Xóa chương trình cashback “${program.name}”?`))return;cashbackProgramSnapshots.delete(cashbackProgramSnapshotKey(program));state.cashbackProgramGroups=state.cashbackProgramGroups.filter(item=>item.id!==program.id);cashbackProgramSelection.programId="";cashbackProgramSelection.packageId="";saveState("Đã xóa chương trình cashback");});
  root.querySelector("[data-card-total-enabled]")?.addEventListener("change",event=>{const amount=root.querySelector("[data-card-total-min]");if(amount)amount.disabled=!event.target.checked;});
...
function ensureCashbackProgramsForSelectedPeriod(){
  ...
  state.cashbackProgramGroups=result.programs;
...
function transactionCrudHandlers(entity,{personalAdd=false}={}){
  ...
  return {
    ...
    remove:id=>{if(!confirm("Xóa giao dịch đã chọn?"))return;state.transactions=state.transactions.filter(transaction=>transaction.id!==id);clearRowSelection(entity);saveState("Đã xóa giao dịch");},
    bulkRemove:ids=>{const selected=new Set(ids);state.transactions=state.transactions.filter(transaction=>!selected.has(transaction.id));clearRowSelection(entity);saveState(`Đã xóa ${ids.length} giao dịch`);}
  };
}
function renderOrderTransactions(monthlyRows){
  const entity="transactions";
  const source=orderTransactions(monthlyRows);
...
function renderTransactions(){
  const monthlyRows=[...periodTx()].sort(compareTransactionsNewestFirst);
  const view=activeTransactionChildTab==="personal"?renderPersonalTransactions(monthlyRows):renderOrderTransactions(monthlyRows);
  document.querySelector("#view-transactions").innerHTML=`<div class="card transactions-card"><div class="section-title"><h2>Danh sách giao dịch</h2><small>${view.rows.length}/${view.source.length} dòng trong tháng</small></div>${transactionChildTabs()}${view.toolbar}${view.table}</div>`;
  document.querySelectorAll("[data-transaction-child-tab]").forEach(button=>button.addEventListener("click",()=>{const next=button.dataset.transactionChildTab;if(next===activeTransactionChildTab)return;closeFilterPanelWithoutApply("transactions");activeTransactionChildTab=next;closeTableContextMenu();renderAll();}));
  wireToolbar(view.entity,transactionCrudHandlers(view.entity,{personalAdd:activeTransactionChildTab==="personal"}));
}
...
function exportTransactionsRows(rows){
  ...
  return rows.map(t=>({
    ...
    "Thời gian": normalizeTransactionTime(t.transactionTime),
    "Thẻ": t.cardId,
...
function exportCardsRows(){
  return [...(state.cards||[])].sort((a,b)=>compareVietnameseText(cardBankName(a),cardBankName(b))||compareVietnameseText(a.id,b.id)).map(c=>({
    ...
    "Số ngày thanh toán":normalizePaymentTermDays(c.paymentTermDays) ?? "",
...
function exportProgramsRowsLegacy(){
  return [...(state.cashbackProgramGroups||[])].sort((a,b)=>(a.year||0)-(b.year||0)||(a.month||0)-(b.month||0)||compareVietnameseText(a.cardId,b.cardId)||compareVietnameseText(a.name,b.name)).flatMap(program=>{
    ...
    "Năm":program.year||"","Tháng":program.month||"","Ngân hàng":cashbackProgramBankName(program),"Card ID":program.cardId||"","Program ID":program.id,"Tên chương trình":program.name||"","Package ID":pkg?.id||"","Tên gói":pkg?.name||"",
    "Group ID":group.id,"Tên nhóm":group.name||"","Tổng chi tối thiểu":group.totalSpendMinimum??"","Tổng chi tối thiểu toàn chương trình":program.totalSpendMinimum??"","Max cashback toàn kỳ":program.maxCashbackPerPeriod??"","Số lần đổi gói tối đa":program.packageSwitchLimit??"","Điều kiện kết hợp":normalizeCombineOperator(group.conditionCombination),"Ghi chú chung":group.note||"",
...
function exportProgramsRows(){
  return exportCashbackProgramRows(state.cashbackProgramGroups||[],{mccCategories:state.mccCategories,bankName:cashbackProgramBankName,cardCashbackConfigs:state.cashbackCardConfigs});
}
...
function cardActivationUpdates(workbook){
  ...
  return rows.map((row,index)=>{
    ...
    if(hasPaymentTerm&&id&&rawTerm==="") throw new Error(`Sheet “Thẻ”, dòng ${index+2}: Số ngày thanh toán không được để trống.`);
    if(hasPaymentTerm&&rawTerm!==""&&paymentTermDays==null) throw new Error(`Sheet “Thẻ”, dòng ${index+2}: Số ngày thanh toán phải là số nguyên dương.`);
...
function buildImportedCashbackGroupsLegacy(rows){
  if(rows.some(row=>normalizeImportText(row["Package ID"]||row["Tên gói"]))){
    ...
    rows.forEach((row,index)=>{
      const cardId=normalizeImportText(row["Card ID"]),name=normalizeImportText(row["Tên chương trình"]??row["Tên nhóm"]??row["Chương trình"]),year=Number(row["Năm"]),month=Number(row["Tháng"]),programId=normalizeImportText(row["Program ID"])||buildCashbackProgramId(cardId,`${name}-${year}-${month}`);
      if(!cardId||!state.cards.some(card=>card.id===cardId))throw new Error(`Sheet “Chương trình Cashback”, dòng ${index+2}: Card ID không hợp lệ.`);if(!name)throw new Error(`Sheet “Chương trình Cashback”, dòng ${index+2}: Tên chương trình không được để trống.`);
      if(!programs.has(programId))programs.set(programId,{id:programId,cardId,name,year,month,totalSpendMinimum:normalizeMoney(row["Tổng chi tối thiểu toàn chương trình"],{emptyValue:null}),maxCashbackPerPeriod:normalizeMoney(row["Max cashback toàn kỳ"],{emptyValue:null}),packageSwitchLimit:Number(row["Số lần đổi gói tối đa"])||0,packages:[],packageHistory:[]});
      const program=programs.get(programId),packageId=normalizeImportText(row["Package ID"])||`${programId}-PACKAGE-${program.packages.length+1}`,packageName=normalizeImportText(row["Tên gói"]);
      let pkg=program.packages.find(item=>item.id===packageId);if(!pkg){pkg={id:packageId,name:packageName||`Gói ${program.packages.length+1}`,groups:[]};program.packages.push(pkg);}
      const groupName=normalizeImportText(row["Tên nhóm"]),groupId=normalizeImportText(row["Group ID"])||`${packageId}-GROUP-${pkg.groups.length+1}`;let group=pkg.groups.find(item=>item.id===groupId);if(!group){group={id:groupId,name:groupName,totalSpendMinimum:normalizeMoney(row["Tổng chi tối thiểu"],{emptyValue:null}),conditionCombination:normalizeCombineOperator(row["Điều kiện kết hợp"]),note:normalizeImportText(row["Ghi chú chung"]),conditions:[]};pkg.groups.push(group);}
      const conditionId=normalizeImportText(row["Condition ID"])||`${groupId}-COND-${group.conditions.length+1}`;if(usedConditionIds.has(conditionId))throw new Error(`Sheet “Chương trình Cashback”, dòng ${index+2}: Condition ID bị trùng.`);usedConditionIds.add(conditionId);
      const mccCodes=normalizeImportText(row["Mã MCC"]).split(",").map(value=>value.trim()).filter(Boolean),allMcc=viKey(row["Nhóm MCC"])==="tat ca"||viKey(row["Mã MCC"])==="tat ca"||!mccCodes.length,maxText=normalizeImportText(row["Max CB"]),maxCashbackUnlimited=viKey(maxText)==="khong gioi han";
      group.conditions.push({id:conditionId,name:normalizeImportText(row["Tên điều kiện"]??groupName),rate:parseCashbackRateInput(row["% CB"]),max:maxCashbackUnlimited?null:normalizeMoney(row["Max CB"],{emptyValue:0}),maxCashbackUnlimited,eligibleSpendMinimum:normalizeMoney(row["Chi nhóm tối thiểu"]??row["Chi nhóm để max"],{emptyValue:null}),channel:normalizeTransactionMethod(row["Hình thức giao dịch"]),allMcc,mccCategoryIds:allMcc?[]:mccCodes.map(code=>state.mccCategories.find(item=>String(item.mcc)===code)?.id).filter(Boolean),note:normalizeImportText(row["Ghi chú điều kiện"])});
    });
    return [...programs.values()];
  }
  const groups=new Map(),usedConditionIds=new Set();
  rows.forEach((row,index)=>{
    const cardId=normalizeImportText(row["Card ID"]),name=normalizeImportText(row["Tên nhóm"]??row["Chương trình"]);
    if(!cardId||!state.cards.some(card=>card.id===cardId))throw new Error(`Sheet “Chương trình Cashback”, dòng ${index+2}: Card ID không hợp lệ.`);
    if(!name)throw new Error(`Sheet “Chương trình Cashback”, dòng ${index+2}: Tên nhóm không được để trống.`);
    const year=Number(row["Năm"]),month=Number(row["Tháng"]),groupId=normalizeImportText(row["Group ID"])||buildCashbackProgramId(cardId,`${name}-${year}-${month}`);
    const key=groupId||`${cardId}|${year}|${month}|${viKey(name)}`;
    if(!groups.has(key))groups.set(key,{id:groupId,cardId,name,year,month,totalSpendMinimum:normalizeMoney(row["Tổng chi tối thiểu"]??row["Chỉ tiêu tổng"],{emptyValue:null}),conditionCombination:normalizeCombineOperator(row["Điều kiện kết hợp"]),note:normalizeImportText(row["Ghi chú chung"]),conditions:[]});
    const group=groups.get(key),conditionName=normalizeImportText(row["Tên điều kiện"]??name);
    let conditionId=normalizeImportText(row["Condition ID"])||`${group.id}-COND-${group.conditions.length+1}`;
    if(usedConditionIds.has(conditionId))throw new Error(`Sheet “Chương trình Cashback”, dòng ${index+2}: Condition ID bị trùng.`);
    usedConditionIds.add(conditionId);
    const mccCodes=normalizeImportText(row["Mã MCC"]).split(",").map(value=>value.trim()).filter(Boolean);
    const allMcc=viKey(row["Nhóm MCC"])==="tat ca"||viKey(row["Mã MCC"])==="tat ca"||!mccCodes.length;
    const mccCategoryIds=allMcc?[]:mccCodes.map(code=>state.mccCategories.find(item=>String(item.mcc)===code)?.id).filter(Boolean);
    const maxText=normalizeImportText(row["Max CB"]),maxCashbackUnlimited=viKey(maxText)==="khong gioi han";
